...
(function () {
  ...
  function scanPage() {
    ...
    // marketing: airlines that bury their phone number behind 4 chatbot
    // prompts, telcos that gate everything behind a login, banks whose
    // contact page is a 500 KB JS app. When the user is on one of these
    // hosts, we surface the canonical support contact directly from the
    // override library -- no scraping required, no chatbot to defeat.
    // On-page finds still run and win on ties via canonical phoneKey
    // dedup, so an override that's gone stale gets replaced by what we
    // find on the live page.
