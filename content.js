...
(function () {
  ...
  function scanPage() {
    ...
    // nested types. Runs before the inline-script pass so this higher-
    // confidence, author-declared source wins attribution on any overlap.
    extractJsonLdContacts(results, seen);

    // 1d. Scan inline <script> body text. Complements scanPageGlobals():
    //
  ...
  // Lifted to module scope so the document-level mousemove / mouseup
  // handlers installed once on first render can see it, and so the
  // drag-vs-click suppression flag survives the shadow re-render between
  // mouseup and the trailing click event the browser dispatches.
  ...
  function spWireEvents(shadow, currentResults) {
    ...
    // Wire vertical drag-to-reposition on the collapsed tab. Re-attached
    // on every render since the tab DOM is rebuilt each ensureSidePanel
    // pass.
